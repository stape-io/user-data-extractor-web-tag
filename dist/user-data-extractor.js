/*! @stape-io/user-data-extractor v1.0.0 | 2025-10-31T14:55:01.648Z */
var extractUserDataAuto = (function () {
  ...
  function runUserDataExtraction(config) {
    ...
    if (extract.email) {
      const emailCandidates = extractEmails(selection);
      const bestCandidate = pickBest(emailCandidates);
      if (bestCandidate) {
    ...
    if (extract.phone) {
      const phoneCandidates = extractPhones(selection);
      const bestCandidate = pickBest(phoneCandidates);
      if (bestCandidate) {
    ...
    if (extract.city) {
      const cityCandidates = extractCities(selection);
      const bestCandidate = pickBest(cityCandidates);
      if (bestCandidate) {
    ...
    if (extract.country) {
      const countryCandidates = extractCountries(selection);
      const bestCandidate = pickBest(countryCandidates);
      if (bestCandidate) {
  ...
  function extractEmails(selection, config) {
    ...
    const accept = (normalized) => {
      ...
      if (host && belongsToHost(normalized, host)) return false;
      return true;
    };
